<template style="padding: 20px">
  <div>
    ...
    <el-table :data="tableData" border style="width: 100%" max-height="500">
      <el-table-column fixed prop="initiaLatitude" label="起点纬度" />
      <el-table-column prop="initiaLongitude" label="起点经度" />
      <el-table-column prop="targetLatitude" label="终点纬度" />
      <el-table-column prop="targetLongitude" label="终点经度" />
      <el-table-column prop="width" label="宽度" />
      <el-table-column prop="heightLeft" label="左高度" />
      <el-table-column prop="heightRight" label="右高度" />
      <el-table-column prop="mongoID" label="对应的mongoid" />
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="终点维度" prop="targetLatitude">
          <el-input
            v-model="form.targetLatitude"
            type="number"
            step="0.00001"
            max="999.99999"
            min="0.00000"
        ...
        <el-form-item label="终点经度" prop="targetLongitude">
          <el-input
            v-model="form.targetLongitude"
            type="number"
            step="0.00001"
            max="999.99999"
            min="0.00000"
        ...
        <el-form-item label="左高度" prop="heightLeft">
          <el-input
            v-model="form.heightLeft"
            type="number"
            min="0"
            placeholder="请输入正整数"
          />
        </el-form-item>
        <el-form-item label="右高度" prop="heightRight">
          <el-input
            v-model="form.heightRight"
            type="number"
            min="0"
            placeholder="请输入正整数"
          />
        </el-form-item>
        <el-form-item label="mongoID" prop="mongoID">
          <el-input
...
import { centralTrackToExcell, centralTrackToCsv } from '@/api/ordering'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        initiaLatitude: [
          { required: true, message: '请输入起点经度', trigger: 'blur' }
        ],
        initiaLongitude: [
          { required: true, message: '请输入起点维度', trigger: 'blur' }
        ],
        targetLatitude: [
          { required: true, message: '请输入终点经度', trigger: 'blur' }
        ],
        targetLongitude: [
          { required: true, message: '请输入终点维度', trigger: 'blur' }
        ],
        width: [
          { required: true, message: '请输入宽度', trigger: 'blur' }
        ],
        heightLeft: [
          { required: true, message: '请输入左高度', trigger: 'blur' }
        ],
        heightRight: [
          { required: true, message: '请输入右高度', trigger: 'blur' }
        ],
        mongoID: [
          { required: true, message: '请输入mongoID', trigger: 'blur' }
        ]
      },
      tableData: []
    }
  ...
  methods: {
    handleDelete(routeID) {
      ...
      request.delete('api/centraltrack/' + routeID).then(res => {
        if (res.code === '200') {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    exportData() { // 导出.xlsx格式数据
      centralTrackToExcell().then((data) => {
    ...
    downloadFile(data) {
      ...
      link.setAttribute('download', '民航中心航线表' + '.xlsx')
    ...
    downloadFileCsv(data) {
      ...
      link.setAttribute('download', '民航中心航线表' + '.csv')
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      // 友好型提示
      ...
      this.$refs[form].validate((valid) => {
        if (!valid) {
          ...
          if (this.form.routeID) { // id存在，为更新
            request.put('api/centraltrack', this.form).then(res => {
              console.log(res)
              if (res.code === '200') {
                this.$message({
                  type: 'success',
            ...
            request.post('api/centraltrack', this.form).then(res => {
              console.log(res)
              if (res.code === '200') {
                this.$message({
                  type: 'success',
