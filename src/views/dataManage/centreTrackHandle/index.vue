<template style="padding: 20px">
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      -->
      <el-button type="primary" @click="matchMedia()">匹配</el-button>
    ...
    <el-table :data="tableData" border style="width: 100%" max-height="500">
      <el-table-column fixed prop="_id" label="数据标识" />
      <el-table-column prop="target_id" label="目标标识" />
      <el-table-column prop="track_point_items.length" label="目标轨迹点数" />
      <el-table-column prop="match_result.0.id" label="匹配结果"/>
      <el-table-column prop="match_result.0.similarity" label="匹配相似度"/>
    </el-table>
...
export default {
  ...
  methods: {
    handleDelete(routeID) {
      ...
      request.delete('/api/matchtrack/' + routeID).then(res => {
        if (res.code === '0') {
          this.$message({
            type: 'success',
            message: '删除成功'
      ...
      this.$refs[form].validate((valid) => {
        if (!valid) {
          ...
          if (this.form.routeID) { // id存在，为更新
            request.put('/api/matchtrack', this.form).then(res => {
              console.log(res)
              if (res.code === '0') {
                this.$message({
                  type: 'success',
            ...
            request.post('/api/matchtrack', this.form).then(res => {
              console.log(res)
              if (res.code === '0') {
                this.$message({
                  type: 'success',
