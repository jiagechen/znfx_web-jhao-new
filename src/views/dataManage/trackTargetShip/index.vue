...
export default {
  ...
  data() {
    return {
      ...
      actionUrl: 'dev/api/tracktarget/upload', // 上传的后台地址
  ...
  methods: {
    ...
    load() {
      request.get('api/tracktarget/ship', {
        params: {
    ...
    async check(key) {
      var res = await this.$http.get('dev/api/tracktarget/check', {
        params: { 'key': key }
      })
    ...
    async recursionUpload(param, file) {
      ...
      console.log(param)
      var res = await this.$http.post('dev/api/tracktarget/upload', totalParam, config)
