<template style="padding: 20px">
  <div>
    ...
    <el-table :data="tableData" border style="width: 100%" max-height="500">
      ...
      <el-table-column prop="targetLatitude" label="终点经度" />
      <el-table-column prop="targetLongitude" label="终点纬度" />
      <el-table-column prop="width" label="宽度" />
      <el-table-column prop="mongoid" label="mongoid" />
...
import { shipCentralTrackToExcell, shipCentralTrackToCsv } from '@/api/ordering'
...
export default {
  ...
  methods: {
    handleDelete(routeID) {
      ...
      request.delete('api/shipcentraltrack/' + routeID).then(res => {
        if (res.code === '200') {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    load() {
      request.get('api/shipcentraltrack', {
        params: {
    ...
    exportData() { // 导出.xlsx格式数据
      shipCentralTrackToExcell().then((data) => {
    ...
    downloadFile(data) {
      ...
      link.setAttribute('download', '民船中心航线表' + '.xlsx')
    ...
    exportDataCsv() { // 导出文件(.csv)
      shipCentralTrackToCsv().then((data) => {
    ...
    downloadFileCsv(data) {
      ...
      link.setAttribute('download', '民船中心航线表' + '.csv')
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      // 友好型提示
      ...
      this.$refs[form].validate((valid) => {
        if (!valid) {
          ...
          if (this.form.routeID) { // id存在，为更新
            request.put('api/shipcentraltrack', this.form).then(res => {
              console.log(res)
              if (res.code === '200') {
                this.$message({
                  type: 'success',
            ...
            request.post('api/shipcentraltrack', this.form).then(res => {
              console.log(res)
              if (res.code === '200') {
                this.$message({
                  type: 'success',
