...
export default {
  ...
  methods: {
    handleDelete(fragmentId) {
      ...
      request.delete('api/shiptrack/' + fragmentId).then(res => {
        if (res.code === '200') {
          this.$message({
            type: 'success',
            message: '删除成功'
      ...
      this.$refs[form].validate((valid) => {
        if (!valid) {
          ...
          if (this.form.fragmentId) { // id存在，为更新
            request.put('api/shiptrack', this.form).then(res => {
              console.log(res)
              if (res.code === '200') {
                this.$message({
                  type: 'success',
            ...
            request.post('api/shiptrack', this.form).then(res => {
              console.log(res)
              if (res.code === '200') {
                this.$message({
                  type: 'success',
    ...
    load() {
      request.get('api/shiptrack/all', { // 获取所有数据
        params: {
